...
def test_history_timeline_latency_smoke(api, identity_factory):
    ...
    path = "/history/me?state_scope=verified&limit=50"
    for _ in range(3):
        api.call("GET", path, token=focus["token"])

    iterations = int(os.getenv("HISTORY_PERF_ITERATIONS", "25"))
    durations = []
    ...
    p95_limit_ms = float(os.getenv("HISTORY_P95_THRESHOLD_MS", "700"))
    assert p95_ms <= p95_limit_ms, (
        f"Latencia alta en timeline history. avg_ms={avg_ms:.1f}, "
